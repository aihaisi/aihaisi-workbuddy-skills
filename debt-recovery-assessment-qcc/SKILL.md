...
1. 每个新会话首次执行本 SKILL 时，如客户端支持 MCP Resources，先执行资源发现并读取 `qcc://skills/index`、`qcc://terminology/core`、`qcc://policy/data-discipline`、`qcc://policy/entity-anchoring` 与 `qcc://skill/debt-recovery-assessment/tool-binding`。
...
> **裁判文书详情下钻（确认债权金额 / 代位权依据 · 列表先行 → 单篇）**：第二层金额、以及原告自身债权的金额与依据，**须以裁判文书原文为准**——先由 `get_judgment_debtor_info` / `get_judicial_documents` 列表拿「文书ID」→ 对关键案件单篇调 `mcp__qcc-risk__get_judicial_document_detail`（section 默认 `core` = 判决结果 + 本院认为；代位权线索可加 `claims` 诉辩主张）。**单轮 ≤ 3 篇**；债权金额一律引文书原文，不臆测、不做胜诉率预测（铁律 6）。
...
> **人关联风险先扫（实控人名下企业 · 防散弹枪）**：`mcp__qcc-executive__get_executive_related_risk_scan`（searchKey + personName 双锚）一次返回该法代 / 实控人任职 / 投资 / 控制企业的风险面（三层 L1/L2/L3），用于情境 B/C 判断"实控人名下其他企业是否殷实可作延伸追偿目标，或是否陆续注销 / 异动转移资产"。**禁止**对其控制企业逐个散弹枪 scan；仅对先扫"有风险 / 可执行"的关联企业单点下钻。
...
> **使用约定**：以下是债务清偿能力评估报告的**完整骨架**——标题层级、表头与列、免责声明**全部固定**，模型只把 `{}` 占位替换为工具返回值，**禁止新增 / 删除章节、禁止改表列、禁止虚构接口未返回的列或分类**。各章数据来源见每节标注（一律业务语言，报告内不写工具代码名 / server 名 / 内部维度编号）。每章可在固定表格后附一段"深度推演"业务叙述，但**只能基于已填入的接口原始数字展开，不得引入新数字 / 新主体**。
> **填写纪律（务必遵守，对齐本 SKILL 已有铁律）**：
> ① **数据零重构**——被执行金额 / 各维命中计数（失信 N 条 / 被执行 N 条 / 限高 N 条 / 终本 N 条 / 股权冻结 N 条）/ 涉案金额 / 持股比例等聚合值**逐字引用接口原始返回**，禁自行加总 / 相减 / 相乘 / 加权 / 估算 / 四舍五入圆场；穿透持股比例如涉表决权，逐字照抄接口聚合值（如 53.0011%），**禁把各层持股相乘自算穿透路径百分比**；接口未返回的字段留空或写"未披露 / 本次未核验"，不编造。
> ② **先扫后钻 · 定性须下钻**——§司法执行现状 / §历史偿债 / §实控人兜底的风险面先由企业风险全量扫描分诊命中计数，仅对 `count>0` 维度下钻明细；对任一维度给**定性判断**（如"连年失信型""属正常维权""实控人兜底路径封闭"）前必须已下钻该维度明细，未下钻则只写"N 条（未取明细）"，禁凭计数或印象定性。
> ③ **持股平台必下钻 · 防换壳误判**——§可执行财产线索 / §实控人兜底涉"大股东退出 + 新持股平台进入"时，须下钻该平台合伙人 / 股东判定"换壳不换人 / 真退出"，再给资产异动 / 退出结论，两个方向均须下钻数据支撑。
> ④ **关联单层预警**——实控人名下企业用人关联先扫，单层预警终点，不对返回关联方再自动逐个穿透扫描。
> ⑤ **不替客户决策**——回收率 / 评级须落到数字区间并附推演依据，但客观陈述、属决策支持材料，不替客户拍板"起诉 / 撤诉 / 和解"、不构成对追偿结果的承诺。
...
**所属行业：** {国民经济行业大类}
**现任法定代表人：** {姓名}
**工商登记状态：** {存续 / 注销 / 吊销 / 经营异常}
**报告生成：** YYYY-MM-DD HH:MM:SS
**审计留档编号：** DRA-{统一社会信用代码}-{YYYYMMDD}
**综合追偿评级：** {S / A / B / C / D} 级 · {评级名称} · 综合置信度 {%}
**预期回收率：** {区间，如 15-30%}
**名义债权金额：** {} 元 · **建议申请保全金额：** {} 元
...
> **一句话结论：** {被告是否资不抵债、当前债务压力规模、有无可执行标的、实控人兜底是否可行、给什么评级与回收率}

| 关键判断 | 结论 | 置信度 | 证据来源 |
| --- | --- | --- | --- |
| 是否资不抵债 | {资不抵债 / 安全 / 无财务数据} | {%} | {资产负债率 / 所有者权益逐字引用} |
| 流动性偿债能力 | {充裕 / 紧张 / 归零} | {%} | {速动比率 / 流动比率} |
| 当前可执行资产 | {充足 / 有限 / 近乎零} | {%} | {可保全标的扫描结果} |
| 当前债务压力 | {轻 / 中 / 重} | {%} | {被执行 N 条 · 失信 N 条逐字引用} |
| 历史偿债模式 | {模式 A/B/C/D/E} | {%} | {历史失信 / 被执行分桶} |
| 实控人兜底路径 | {可行 / 受限 / 封闭} | {%} | {实控人个人司法状态} |
| 综合追偿评级 | {S / A / B / C / D} | — | {综合} |

**建议申请保全金额：** {} 元（占名义债权 {}%）

**推荐 Action（按 T+0 / T+3 / T+7 / T+14 / T+30 紧迫度）：**
1. **[T+0]** {……}
2. **[T+3]** {……}
3. **[T+7]** {……}
4. **[T+14]** {……}
...
{综合追偿评级 + 关键判断依据 + 名义债权与建议保全金额 + 追偿可行性建议（起诉 / 债权申报 / 放弃由客户决策），3-5 句业务语言}

## 2 数据来源与互证方法

| 维度 | 数据来源 | 互证方式 |
| --- | --- | --- |
| 工商 / 股权 / 财务 | 企查查工商登记数据 + 企查查财务数据 | {真实财报与事件信号交叉} |
| 司法执行 / 债务 | 企查查风险信息数据 | {先扫后钻分诊 + 命中下钻} |
| 历史偿债 / 资产异动 | 企查查历史存档数据 | {10 年时间序列回溯} |
| 实控人个人兜底 | 企查查董监高画像数据 | {企业名 + 姓名双锚定} |

> 各维以接口实时返回为准；定性判断须有对应维度下钻明细支撑，未下钻只陈述计数。
...
> 状态拐点：注销 → 回收路径切换为刺破面纱 + 追缴出资；破产受理 → 立即标注债权申报期；经营异常 / 吊销 → 预判是否进入注销流程。

### 3.2 真实财务底盘（逐字引用财务数据）

| 项目 | 最新年度数值 | 行业正常值 | 风险信号 |
| --- | --- | --- | --- |
| 营业总收入 | {} | — | {} |
| 净利润 | {} | 正 | {} |
| 总资产 | {} | — | {} |
...
| 优先级 | 资产类型 | 具体标的 | 登记状态 | 他项权利 / 是否已被冻结 | 可保全净值 | 执行难度 | 建议保全顺位 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| {1} | {现金 / 股权 / 不动产 / 动产 / 知产 / 应收} | {} | {} | {未设定 / 已抵押 / 已冻结 N 条} | {} | {★~★★★★★} | {} |


### 4.2 对外投资股权（可执行 / 已冻结）

| 序号 | 被投资企业 | 持股比例 | 认缴 / 出资 | 是否已出质 / 冻结 | 可执行性 |
| --- | --- | --- | --- | --- | --- |
| 1 | {} | {%} | {} | {未出质 / 已冻结 N 条} | {} |
...
| 资产类型 | 接口返回 | 抵押 / 查封状态 | 剩余可执行价值 |
| --- | --- | --- | --- |
| 土地抵押 | {N 条 / 无} | {} | {} |
| 动产抵押 | {N 条 / 无} | {} | {} |
| 知识产权出质 | {N 条 / 无} | {} | {} |

**资产异动预警：** {如近 6-12 月出现大规模股权转让 / 抵押异常注销 / 知产出质异动 → 写"疑似资产转移规避债务，建议评估撤销权之诉"；逐项引用历史接口返回，下钻确认后再定性}

## 5 司法执行现状 · 当前债务压力扫描

### 5.1 债务压力分诊（先扫 · 各维命中计数）

| 维度 | 命中计数 | 涉案金额（接口返回） | 最新日期 |
| --- | --- | --- | --- |
| 失信被执行人 | {N 条 / 无} | {逐字引用} | {} |
| 被执行人 | {N 条 / 无} | {逐字引用} | {} |
| 限制高消费 | {N 条 / 无} | {逐字引用 / 未披露} | {} |
| 终本案件 | {N 条 / 无} | {逐字引用} | {} |
| 股权冻结 | {N 条 / 无} | {逐字引用} | {} |
| 欠税 / 对外担保 | {N 条 / 无} | {逐字引用} | {} |


### 5.2 重大被执行案件（仅 count>0 下钻 · 金额引文书原文）

| 排序 | 案号 | 执行法院 | 立案日期 | 执行标的 | 状态 |
| --- | --- | --- | --- | --- | --- |
| 1 | {} | {} | YYYY-MM-DD | {逐字引用} | {失信 / 恢复执行 / 终本} |


### 5.3 债务压力三层结构 × 清偿率测算

- **第一层 · 显性优先顺位债务**（欠税 / 职工工资 / 有抵押担保债权）：{逐项引用接口返回金额；接口未返回则写"未单独查询 / 未披露"，不估算填数}
- **第二层 · 已判决未执行的等同顺位债权池**：{被执行案件涉案金额，逐字引用，不自行加总跨案金额}
- **第三层 · 潜在债务**（对外担保 / 未结案件）：{逐项列举，不量化估算}

**清偿率测算：** {给出回收率区间（如 15-30%）+ 推演依据 + 保守 / 激进假设对比；测算所用各项金额须为接口返回原始值或文书原文，差额不圆场为"四舍五入"}

## 6 历史偿债模式识别

### 6.1 历史事件时间序列（按年分桶）

| 期间 | 失信新增 | 被执行新增 | 终本新增 | 阶段判断 |
| --- | --- | --- | --- | --- |
| {年份区间} | {N / 罕见} | {N} | {N} | {平稳 / 萌芽 / 爆发 / 持续 / 稳态} |
...
| 模式 | 定义 | 本企业符合度 |
| --- | --- | --- |
| A · 始终清洁型 | 10 年零失信 / 被执行 / 终本 | {✅ / ❌} |
| B · 修复型 | 历史出险已全部修复、近 3 年清洁 | {✅ / ❌} |
| C · 间歇失信型 | 每 2-3 年一轮、最终部分履行 / 终本 | {✅ / ❌} |
| D · 连年失信型 | 近 5 年连续每年新增失信 / 被执行 | {✅ / ❌} |
| E · 集中爆发型 | 近 12-24 月突然爆发（此前 5 年清洁） | {✅ / ❌} |

**模式结论：** {模式 X} · 对应预期回收率区间 {} · 推荐保全激进度 {激进 / 标准 / 保守}。{判断依据须为已下钻的具体数量 / 时间 / 金额，未下钻只写计数}

## 7 法代 / 实控人个人兜底评估

> 每位法代 / 实控人独立成小节；个人风险先扫（企业名 + 姓名双锚定）→ 仅对命中维度下钻；人关联单层预警，不对其控制企业逐个自动扫描。

### 7.{n} {姓名}（{现任法代 / 实际控制人 / 已卸任}）
...
| 个人限制出境 | {N 条 / 无} | {} |
| 个人名下股权冻结 | {N 条 / 无} | {} |
| 个人对外投资 / 控制企业 | {N 家 / 无} | {人关联先扫结果，单层} |

**情境落点：** {情境 A 企业存续+实控人清洁 / 情境 B 企业壳化+实控人殷实 / 情境 C 企业存续+实控人已跑路或被追责} · {刺破面纱可行性初判；属数据层面线索，法律论证由承办律师完成}

## 8 综合追偿评级 × 保全标的清单 × 推荐 Action

### 8.1 综合追偿评级

| 评级 | 触发条件（回收率） | 本案符合度 |
| --- | --- | --- |
| S · 强烈追偿 | 财务+资产+历史+实控人全绿 · 60-85% | {✅ / ❌} |
| A · 积极追偿 | 财务中等 或 历史已修复 · 40-65% | {✅ / ❌} |
| B · 有限追偿 | 有压力 但仍有可保全资产 / 兜底可行 · 20-40% | {✅ / ❌} |
| C · 困难追偿 | 连年失信 / 资产大量异动 / 他人先行保全 · 10-25% | {✅ / ❌} |
| D · 不建议追偿 | 已注销 / 已破产 / 净值低于诉讼成本 / 实控人跑路 · < 10% | {✅ / ❌} |

**评级结论：** {S / A / B / C / D} 级 · 预期回收率 {区间}。

### 8.2 按优先级排序的保全标的清单（S / A / B 级出具）

| 优先级 | 资产类型 | 具体标的 | 登记状态 | 评估净值 | 执行难度 | 建议保全金额 |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | {} | {} | {} | {} | {★} | {} |

**合计建议保全金额：** {} 元（占名义债权 {}%）。{D 级写"不建议启动保全 / 仅建议债权申报保留时效"}
...
> 行动清单为决策支持，立案 / 撤诉 / 和解 / 放弃由委托人与承办律师决策，本报告不替客户拍板、不承诺追偿结果。

---

## 数据来源与免责声明

**数据来源：** 本报告全部数据由企查查 MCP 实时返回（上游为国家市场监督管理总局及省 / 市市场监管、人民法院执行信息公开网、企业披露年报等公示数据），采集时间 YYYY-MM-DD HH:MM:SS。

**免责声明：**
1. 财务数据来源于企业年报披露，数据口径 / 真实性以企业实际披露为准；对已进入危机的主体，年报可能含强调事项段 / 保留意见，正式尽调应由专业律师 + 注册会计师对年报实质审查。
2. 可执行资产估值基于公开登记信息与行业经验估算，并非司法评估价；真正的清偿顺位、查封金额、拍卖价格以法院实际处置为准。
3. 刺破公司面纱追究实控人责任涉及复杂法律证据组织（抽逃出资 / 人格混同 / 滥用有限责任等），本报告仅提供数据层面线索识别与路径可行性初判，具体法律论证应由承办律师结合卷宗实质审查完成。
4. 追偿成功率预测基于历史数据统计经验，个案实际结果受法官裁量、诉讼时机、被告应诉策略、跨境执行可行性等多重因素影响；本报告属决策支持材料，不构成对追偿结果的承诺。
```

> **章节 ↔ 工具绑定**：执行摘要←全维度汇总；§3←`get_company_registration_info` / `get_financial_data` / `get_shareholder_info` / `get_actual_controller`（注销 / 破产看 `get_cancellation_record_info` / `get_bankruptcy_reorganization`）；§4←`get_chattel_mortgage_info` / `get_stock_pledge_info` / `get_equity_pledge_info` / `get_land_mortgage_info` / `get_equity_freeze` / `get_ipr_pledge` / `get_external_investments` / `get_branches` + qcc-history 历史抵押 / 投资识别资产异动；§5←`get_company_risk_scan` 先扫 + 命中维度原子下钻（`get_judgment_debtor_info` / `get_dishonest_info` / `get_high_consumption_restriction` / `get_terminated_cases` / `get_guarantee_info` / `get_tax_arrears_notice`）+ 关键案件 `get_judicial_document_detail`；§6←qcc-history（`get_historical_dishonest` / `get_historical_judgment_debtor` / `get_historical_terminated_cases` 等）；§7←`get_executive_risk_scan` 先扫 + 命中 `get_executive_*` 下钻 + `get_executive_controlled_companies` / `get_executive_related_risk_scan` 人关联单层。
...
{"version":1,"steps":[{"id":"debt-entity","label":"确认债务主体","cap":"主体识别","capabilityKeys":["entity"],"completeOn":"route_match"},{"id":"debt-registration","label":"核验主体存续状态","cap":"工商数据","capabilityKeys":["registration"]},{"id":"debt-risk","label":"扫描执行与司法风险","cap":"风险数据","capabilityKeys":["risk"]},{"id":"debt-operation","label":"评估持续经营状况","cap":"经营数据","capabilityKeys":["operation"]},{"id":"debt-history","label":"分析历史风险变化","cap":"历史数据","capabilityKeys":["history"]}]}
