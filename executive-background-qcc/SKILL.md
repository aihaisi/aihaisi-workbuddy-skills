...
>
> 核心能力：
> - 个人司法风险 × 历史追溯：**先扫后钻**——一次扫描该人 18 项当前个人风险维度（`mcp__qcc-executive__get_executive_risk_scan` 双锚分诊）、命中再下钻；历史 14 维按需追溯。覆盖失信 / 限高 / 限出境 / 股权冻结 / 股权出质 / 税收违法 / 行政处罚 / 裁判文书等
...
>
> **风险核查采用「先扫后钻」（企业 + 个人 双主体）**：企业主体先调企业风险全量扫描一次分诊 35 项风险维度；每位董监高/法代/实控人先调 `mcp__qcc-executive__get_executive_risk_scan`（双锚）一次分诊 18 项个人风险维度——再各自对命中维度深入取证，既不漏维度，也避免逐项无效查询。
...
**现状个人风险扫描（先扫后钻 · 2026-06-08 · 对齐 A 层铁律 5 个人维度 · 用 `mcp__qcc-executive__get_executive_risk_scan` 替代逐项散弹枪）**：
...
> 2. **第 2 步 · 下钻（后钻）**：仅对 `count>0` 的维度，调对应原子工具取明细（映射见下表）。示例：scan 显示「失信 1、限高 1、其余 0」→ 只下钻 `mcp__qcc-executive__get_executive_dishonest` + `mcp__qcc-executive__get_executive_high_consumption_ban`。
...
| --- | --- |
| 身份限制与失信 | 失信 `mcp__qcc-executive__get_executive_dishonest` · 限高 `mcp__qcc-executive__get_executive_high_consumption_ban` · 限出境 `mcp__qcc-executive__get_executive_exit_restriction` · 财产悬赏 `mcp__qcc-executive__get_executive_property_reward_notice` |
| 执行与资产冻结 | 被执行 `mcp__qcc-executive__get_executive_judgment_debtor` · 终本 `mcp__qcc-executive__get_executive_terminated_cases` · 股权冻结 `mcp__qcc-executive__get_executive_equity_freeze` · 股权出质 `mcp__qcc-executive__get_executive_equity_pledge` · 股票质押 `mcp__qcc-executive__get_executive_stock_pledge` · 询价评估 `mcp__qcc-executive__get_executive_valuation_inquiry` |
| 行政与税务 | 行政处罚 `mcp__qcc-executive__get_executive_admin_penalty` · 税收违法 `mcp__qcc-executive__get_executive_tax_violation` |
| 司法程序 | 立案 `mcp__qcc-executive__get_executive_case_filing` · 开庭 `mcp__qcc-executive__get_executive_hearing_notice` · 法院公告 `mcp__qcc-executive__get_executive_court_notice` · 送达 `mcp__qcc-executive__get_executive_service_notice` · 裁判文书 `mcp__qcc-executive__get_executive_judicial_docs` · 诉前调解 `mcp__qcc-executive__get_executive_pre_litigation_mediation` |
...
- **第 1 步 · 人关联风险先扫**：`mcp__qcc-executive__get_executive_related_risk_scan`（searchKey = 企业完整名/USCC + personName = 该背调对象，双锚）—— 一次返回该人任职 / 投资 / 控制 / 受益企业的风险面（三层 L1/L2/L3，L3 每维前 10 命中关联企业定位）。**禁止**先列关联企业、再对每家散弹枪 `mcp__qcc-risk__get_dishonest_info` 等原子。
- **第 2 步 · 结构信息**：`mcp__qcc-executive__get_executive_controlled_companies` / `mcp__qcc-executive__get_executive_related_companies` / `mcp__qcc-executive__get_executive_investments` / `mcp__qcc-executive__get_executive_beneficial_owner`（+ 对应 `historical` 版本）补关联企业名单 / 角色 / 投资结构；`mcp__qcc-company__get_external_investments` 交叉比对目标企业自身对外投资。
- **第 3 步 · 下钻**：仅对人关联先扫"有风险"的关联企业按需单点下钻（企业 → `mcp__qcc-risk__get_company_risk_scan` / qcc-risk 原子）。**禁对名单里每家自动逐个 scan / 原子**（单层预警终点）。

**分析要点**：

关联企业清单不是简单罗列 —— 必须按"业务重叠度"分级：高重叠（同业务 / 上下游）、中重叠（相关领域）、低重叠（不相关）。高重叠类关联企业须在后续维度五的利益冲突章节单独分析。

关联企业的风险标签（绿 / 黄 / 红）以**第 1 步人关联先扫**（`mcp__qcc-executive__get_executive_related_risk_scan`）的命中结果为准——避免"清洁人员 + 多家高风险关联企业"的隐性风险被忽略；仅对先扫命中"有风险"的关联企业再下钻取明细，**不对名单里每家逐个扫描**。
...
{"version":1,"steps":[{"id":"executive-entity","label":"确认企业主体","cap":"主体识别","capabilityKeys":["entity"],"completeOn":"route_match"},{"id":"executive-management","label":"核验董监高背景","cap":"董监高数据","capabilityKeys":["management"]},{"id":"executive-risk","label":"扫描人员关联风险","cap":"风险数据","capabilityKeys":["risk"]},{"id":"executive-history","label":"核验任职历史","cap":"历史数据","capabilityKeys":["history"]}]}
