...
marketplace 安装的技能，其 `_skillhub_meta.json` 会多一个 `iconLocalPath` 字段，值是**本机绝对路径**（`C:\Users\<用户名>\...`）。这是真实泄露。

**实测教训（2026-09-18）**：手工把该字段改成 `%USERPROFILE%\...` 并提交后，**约 1 分钟就被 WorkBuddy 重新写回绝对路径**（文件 mtime 自证）。该文件是安装器状态，每次技能扫描都会重新生成。

所以正确处置是**治本**，不是改内容：

```bash
git rm --cached skill_<ID>/_skillhub_meta.json     # 磁盘文件保留，功能不受影响
# 然后写进 .gitignore，注释说明「该字段被自动重写，故不跟踪」
```

判断口径：**`_skillhub_meta.json` 是安装器状态而非技能内容，本就不该进内容库。** 其余 13 个同类文件因为不含绝对路径而稳定，暂可保留；一旦哪个开始出现 `M` 漂移，同样处理。

⚠️ **历史里的泄露无法靠 untrack 消除** —— 旧提交仍含该路径，需要 `git filter-repo` + 强推才能清。用户名属低敏感信息，通常不值得付强推的代价；告知用户事实即可，由他决定。
