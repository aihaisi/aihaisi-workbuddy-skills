...
1. 每个新会话首次执行本 SKILL 时，如客户端支持 MCP Resources，先执行资源发现并读取 `qcc://skills/index`、`qcc://terminology/core`、`qcc://policy/data-discipline`、`qcc://policy/entity-anchoring` 与 `qcc://skill/history-evolution/tool-binding`。
2. 同一会话已成功读取且 checksum 未变化时无需重复读取 Tool Binding；新会话不得沿用上一会话的读取状态。
3. 生成最终报告前重新读取 `qcc://skill/history-evolution/report-template`，并把它作为严格填空骨架；多轮会话后也必须在生成前重读。
...
{"version":1,"steps":[{"id":"history-entity","label":"确认企业主体","cap":"主体识别","capabilityKeys":["entity"],"completeOn":"route_match"},{"id":"history-change","label":"梳理工商历史变更","cap":"历史数据","capabilityKeys":["history"]},{"id":"history-ownership","label":"追踪股权演变","cap":"股权关联数据","capabilityKeys":["ownership"]},{"id":"history-management","label":"追踪管理层变更","cap":"董监高数据","capabilityKeys":["management"]},{"id":"history-operation","label":"追踪经营变化","cap":"经营数据","capabilityKeys":["operation"]}]}
