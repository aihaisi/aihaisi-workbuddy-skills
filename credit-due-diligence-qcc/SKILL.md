...
> **章节 ↔ 工具绑定**：执行摘要←全维度汇总；§3←`verify_company_accuracy` / `get_company_registration_info` / `get_shareholder_info` / `get_actual_controller` / `get_key_personnel`；§4←`get_financial_data` + `get_annual_reports`；§5←`get_company_risk_scan` 先扫 + 命中维度原子下钻（`get_dishonest_info` / `get_judgment_debtor_info` / `get_high_consumption_restriction` / `get_equity_freeze` / `get_equity_pledge_info` / `get_business_exception` 等）+ qcc-history 历史风险工具；§6←qcc-history 信用修复追溯；§7←`get_executive_risk_scan` 先扫 + 命中维度 `get_executive_*` 下钻 + `get_company_related_risk_scan` / `get_executive_related_risk_scan` 关联先扫；§8←§4/§5/§6/§7 汇总。
...
{"version":1,"steps":[{"id":"credit-entity","label":"确认企业主体","cap":"主体识别","capabilityKeys":["entity"],"completeOn":"route_match"},{"id":"credit-registration","label":"核验工商信息","cap":"工商数据","capabilityKeys":["registration"]},{"id":"credit-ownership","label":"穿透股权与关联方","cap":"股权关联数据","capabilityKeys":["ownership"]},{"id":"credit-operation","label":"核验经营状况","cap":"经营数据","capabilityKeys":["operation"]},{"id":"credit-risk","label":"扫描信用与司法风险","cap":"风险数据","capabilityKeys":["risk"]}]}
