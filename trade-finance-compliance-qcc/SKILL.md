...
> - **V2.0 进出口关键人员限出境**（`mcp__qcc-executive__get_executive_exit_restriction`）—— 跨境业务高关注信号，客观列示法代 / 实控人受限状态
> - **V2.0 历史行政处罚追溯**（`mcp__qcc-history__get_historical_admin_penalty`）—— 海关 / 税务历史违规追溯，识别"修复 vs 连年违规"型主体
> - **出口退税相关信号扫描**（`mcp__qcc-risk__get_tax_violation` + `get_tax_arrears_notice`）—— 列示公开税务信号，不直接推断退税资格
...
2. 进出口资质证书
3. **V2.0 新能力：进出口关键人员限出境**（qcc-executive get_executive_exit_restriction —— 跨境业务高关注信号）
...
> **章节 ↔ 工具绑定**：执行摘要←全维度汇总；§3←`get_import_export_credit`（海关信用）/ `get_qualifications`（进出口资质）；§4←`get_executive_exit_restriction`（关键人限出境，双锚定 searchKey + personName）；§5←`get_historical_admin_penalty`（历史行政处罚）；§6←`get_company_risk_scan` 先扫 + `get_tax_violation` / `get_tax_arrears_notice` 等命中维度原子下钻（退税相关公开信号）；§7.1←`get_beneficial_owners`，§7.2 制裁筛查不在本 SKILL 工具能力、须外部工具。
