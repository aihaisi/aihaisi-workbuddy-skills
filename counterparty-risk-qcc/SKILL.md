...
> **章节 ↔ 工具绑定**：执行摘要←全维度汇总；§3←`verify_company_accuracy` / `get_company_registration_info` / `get_shareholder_info` / `get_branches`；§4.1←`get_import_export_credit` / `get_qualifications` / `get_credit_evaluation` / `get_bidding_info` / `get_recruitment_info` / `get_random_check`；§4.2←`get_financial_data`；§4.3←`get_shareholder_info` / `get_actual_controller`（聚合值逐字引用）；§4.4←`get_guarantee_info`；§5.1–5.2←`get_company_risk_scan` 先扫 + 命中维度原子下钻（`get_dishonest_info` / `get_judgment_debtor_info` / `get_high_consumption_restriction` / `get_equity_freeze` / `get_administrative_penalty` / `get_terminated_cases`）；§5.3←`get_external_investments` + 实控人关联（单层预警）；§6←法代 / 实控人个人风险先扫后钻（`get_executive_risk_scan` + 命中下钻 `get_executive_exit_restriction` / `get_executive_dishonest` / `get_executive_high_consumption_ban` / `get_executive_judgment_debtor`）；§7←qcc-history 历届法代 / 历史股东 / 历史行政许可 / 历史行政处罚；§8←全维度汇总 + 敞口分级。
