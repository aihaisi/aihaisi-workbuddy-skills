...
1. 新会话首次执行时，通过标准 `resources/list` 发现并用 `resources/read` 读取核心术语、数据纪律、实体锚定和 `qcc://skill/equity-structure/tool-binding`。
2. 最终报告生成前读取 `qcc://skill/equity-structure/report-template`，严格保持穿透聚合值、固定章节和免责声明。
...
- **① 数字逐字引用**：受益股份 / 表决权 / 总持股比例一律照抄 `get_actual_controller` / `get_beneficial_owners` 接口聚合值（如 47.6955% / 53.0011%），**禁止用各层持股比例相乘自行重构**（大模型多位小数乘法不可靠，实测曾把 79.8674%×11.5446% 算成 9.2145%，正确 9.2204%）。逐路径占比如需展示取自企查查「股权链」（服务端精算），不得自行相乘。
...
| 历史股权变迁 | 企查查历史存档数据 | YYYY-MM-DD | {历届股东 / 高管回溯} |
...
> **章节 ↔ 工具绑定**：执行摘要←全维度汇总；§二 / §三←`get_shareholder_info`（当前股东 / 股本 / 集中度）+ `get_company_registration_info`（工商基础信息，如本 SKILL 未直接声明则取自股东接口随附登记字段）；§四←`get_actual_controller` / `get_beneficial_owners`（聚合值逐字引用）+ `get_executive_controlled_companies`（间接控制中间层）+ `get_executive_beneficial_owner`（自然人反查）；§五←`get_historical_shareholders` / `get_historical_executives`；§六←`get_executive_related_companies` / `get_executive_historical_partners` / `get_external_investments`；§七←§三/§四已填数值判定（不引新数）；§八←`get_external_investments` + UBO 关联企业。
...
{"version":1,"steps":[{"id":"equity-entity","label":"确认企业主体","cap":"主体识别","capabilityKeys":["entity"],"completeOn":"route_match"},{"id":"equity-ownership","label":"分析股权结构","cap":"股权关联数据","capabilityKeys":["ownership"]},{"id":"equity-history","label":"核验股权历史变更","cap":"历史数据","capabilityKeys":["history"]}]}
